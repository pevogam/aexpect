...
def remote_copy(
    ...
    :param login_timeout: The maximal time duration (in seconds) to wait for
            each step of the login procedure (i.e. the "Are you sure" prompt
            or the password prompt)
    :param attempts: Number of attempts to make to deal with transient errors like
...
def nc_copy_between_remotes(
    ...
    Caller-provided sessions are left open.
    ...
    :return: True on success and False on failure.
    """
    close_s_session = s_session is None
    close_d_session = d_session is None
    for attempt in range(1, attempts + 1):
        try:
            check_string = "NCFT"
            ...
            try:
                if close_s_session and s_session is not None:
                    s_session.close()
                    s_session = None
            finally:
                if close_d_session and d_session is not None:
                    d_session.close()
                    d_session = None
    return False
...
def udp_copy_between_remotes(
    ...
    def close_sessions(s_session, d_session):
        """Close both sessions even if closing the source session fails."""
        try:
            if s_session is not None:
                s_session.close()
        finally:
            if d_session is not None:
                d_session.close()
    ...
    for attempt in range(1, attempts + 1):
        ...
        try:
            try:
                ...
                try:
                    if s_session is not None and d_session is not None:
                        stop_server(s_session)
                finally:
                    close_sessions(s_session, d_session)
            return  # transfer is successful
