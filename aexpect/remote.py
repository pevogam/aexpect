...
def rsync_between_remotes(src, dst, port, s_passwd, d_passwd, s_name, d_name,
                          s_path, d_path, directory=True, limit="",
                          log_filename=None, log_function=None, timeout=600,
                          src_inter=None, dst_inter=None):
    """
    Copy files froremote_rsyncm a remote host (guest) to another remote host (guest) through rsync.
    ...
    command += (f" -avz -e 'ssh -p {port} -o UserKnownHostsFile=/dev/null "
                f"-o StrictHostKeyChecking=no' {limit} "
                f"{s_name}@{src}:{quote_path(s_path)} {d_name}@{dst}:{shlex.quote(d_path)}")
    password_list = [s_passwd, d_passwd]
    return remote_copy(command, password_list,
                       log_filename, log_function, timeout)
...
def copy_files_to(address, client, username, password, port, local_path,
    ...
    if client == "scp":
        scp_to_remote(address, port, username, password, local_path,
                      remote_path, directory, limit, log_filename, log_function,
...
def copy_files_from(address, client, username, password, port, remote_path,
    ...
    """
    if client == "scp":
        scp_from_remote(address, port, username, password, remote_path, directory,
                        local_path, limit, log_filename, log_function, timeout,
                        interface=interface)
    elif client == "rsync":
        rsync_from_remote(address, port, username, password, remote_path, directory,
                          local_path, limit, log_filename, log_function, timeout,
                          interface=interface)
    elif client == "rss":
        log_func = None
